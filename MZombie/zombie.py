...
async def aliehi(client: Client, message):
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("اضغط لاستخراج الجلسه", callback_data="generate"),
                InlineKeyboardButton("𝚂𝙾𝚄𝚁𝙲𝙴️", url=f"https://t.me/sourceav"),
            ],
        ]
    )
...
async def makedzombie(client, message):
  ...
  env.write(x)
  env.close()
  os.system(f"cd users/{id} && screen -d -m -S {id} python3 -m FallenMusic")
  oo = [id, Dev]
  Bots.append(oo)
  await message.reply_text("تم تنصيب بوتك بنجاح\n《قناه السورس》»  @sourceav ❤❤️‍🔥")
